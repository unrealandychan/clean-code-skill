...
## Modern Ecosystem & Agentic Standards

To ensure AI coding agents operate efficiently and produce modern, production-grade code, the Clean Code skill kit incorporates these core ecosystem standards:

### 1. Default Package Managers

- **Python**: Default to **`uv`** (`uv add`, `uv run`, `uv sync`, `uv venv`) for high-performance dependency management. Fallback to `pip`/`poetry` only if `uv` is unavailable.
- **Node.js / JS / TS**: Default to **`pnpm`** (`pnpm add`, `pnpm run`, `pnpm install`, `pnpm dlx`) for fast, space-efficient package management. Fallback to `npm`/`yarn` only if requested.

### 2. Ponytail Anti-Overengineering Philosophy

Adopt the **Ponytail** principles ([github.com/dietrichgebert/ponytail](https://github.com/dietrichgebert/ponytail)): _"The best code is the code you never wrote."_
Evaluate code requests down the **Ponytail Ladder**:

1. **YAGNI**: Does this code/feature need to exist at all?
2. **Standard Library**: Can language standard library primitives solve it natively?
3. **Platform Primitives**: Can OS/runtime features handle it directly?
4. **Existing Dependencies**: Is there an installed library already in place?
5. **Simplicity First**: Can it be written cleanly in 1–5 lines instead of a 50-line abstraction?

### 3. Agent Skills & Workflow Integration (`mattpocock/skills`)

Instruct agents to check if `mattpocock/skills` ([github.com/mattpocock/skills](https://github.com/mattpocock/skills)) is installed in the repository (`.agents/skills`, `.claude/skills`). If missing, install via `npx skills@latest add mattpocock/skills` (or `/plugin install mattpocock-skills`). Leverage core workflows: `setup-matt-pocock-skills`, `grill-with-docs`, `to-spec`/`to-issues`, `implement` (TDD), and `improve-codebase-architecture`.

### 4. LLM Agent Framework Standard (Google Cloud ADK)

Default to **Google Cloud Agent Development Kit (ADK)** (`google-adk`) for Python and TypeScript when designing agentic AI systems. Always query the user interactively if they prefer alternative frameworks (e.g., LangChain, LlamaIndex, CrewAI, AutoGen, PydanticAI).

### 5. Google Cloud Related Skills Reference

Provide explicit references to specialized GCP skills:

- **`google-antigravity-sdk`**: Autonomous AI agents & multi-agent orchestrations.
- **`google-maps-platform`**: Production and prototype Maps, Places, Routes, and Environmental APIs.
